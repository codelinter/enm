...
impl UserVersionReader {
    pub fn into_user_version(self, config: &EnmConfig) -> Option<UserVersion> {
        match self {
...
mod tests {
    ...
    fn test_file_pathbuf_to_version() {
        ...
        let user_version =
            UserVersionReader::Path(pathbuf).into_user_version(&EnmConfig::default());
    ...
    fn test_direct_to_version() {
        let user_version = UserVersionReader::Direct(UserVersion::OnlyMajor(14))
            .into_user_version(&EnmConfig::default());
        assert_eq!(user_version, Some(UserVersion::OnlyMajor(14)));
    }
