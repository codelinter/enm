...
impl Version {
    pub fn parse<S: AsRef<str>>(version_str: S) -> Result<Self, node_semver::SemverError> {
        ...
        if lowercased == machine_semver::display_name() {
            Ok(Self::Bypassed)
        } else if lowercased.starts_with("lts-") || lowercased.starts_with("lts/") {
            let lts_type = LongTermType::from(&lowercased[4..]);
    ...
    pub fn installation_path(&self, config: &app_config::AppConfig) -> std::path::PathBuf {
        match self {
            Self::Bypassed => machine_semver::path(),
    ...
    pub fn root_path(&self, config: &app_config::AppConfig) -> Option<std::path::PathBuf> {
...
impl std::fmt::Display for Version {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            Self::Bypassed => write!(f, "{}", machine_semver::display_name()),
