...
impl Version {
    pub fn parse<S: AsRef<str>>(version_str: S) -> Result<Self, node_semver::SemverError> {
        ...
        if lowercased == system_version::display_name() {
            ...
            Ok(Self::VSetter(lowercased))
        }
    }

    pub fn vsetter_name(&self) -> Option<String> {
        match self {
            l @ (Self::Lts(_) | Self::VSetter(_)) => Some(l.v_str()),
            _ => None,
        }
    }
    ...
    pub fn find_aliases(
        &self,
        config: &config::EnmConfig,
    ) -> std::io::Result<Vec<vsetter::StoredVSetter>> {
        let vsetteres = vsetter::list_aliases(config)?
            .drain(..)
            .filter(|vsetter| vsetter.s_ver() == self.v_str())
            .collect();
        Ok(vsetteres)
    }
    ...
    pub fn installation_path(&self, config: &config::EnmConfig) -> std::path::PathBuf {
        match self {
            Self::Bypassed => system_version::path(),
            v @ (Self::Lts(_) | Self::VSetter(_) | Self::Latest) => {
                config.vsetteres_dir().join(v.vsetter_name().unwrap())
            }
    ...
    pub fn root_path(&self, config: &config::EnmConfig) -> Option<std::path::PathBuf> {
...
impl std::fmt::Display for Version {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            Self::VSetter(vsetter) => write!(f, "{vsetter}"),
...
impl PartialEq<node_semver::Version> for Version {
    fn eq(&self, other: &node_semver::Version) -> bool {
        match self {
            Self::Bypassed | Self::Lts(_) | Self::VSetter(_) | Self::Latest => false,
