...
use crate::symlinked::{remove_symlink_dir, shallow_read_symlink, symlink_dir};
...
pub fn list_aliases(config: &AppConfig) -> std::io::Result<Vec<StoredAlias>> {
...
impl std::convert::TryInto<StoredAlias> for &std::path::Path {
    ...
    fn try_into(self) -> Result<StoredAlias, Self::Error> {
        ...
        let destination_path = if shallow_self == machine_semver::path() {
            shallow_self
        } else {
...
impl StoredAlias {
    pub fn s_ver(&self) -> &str {
        if self.destination_path == machine_semver::path() {
            machine_semver::display_name()
        } else {
