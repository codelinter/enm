...
impl UserVersion {
    ...
    pub fn vsetter_name(&self) -> Option<String> {
        match self {
            Self::Full(version) => version.vsetter_name(),
            _ => None,
        }
    }

    pub fn matches(&self, version: &Version, config: &crate::config::EnmConfig) -> bool {
        match (self, version) {
            (Self::Full(a), b) if a == b => true,
            (Self::Full(user_version), maybe_vsetter) => {
                match (user_version.vsetter_name(), maybe_vsetter.find_aliases(config)) {
                    (None, _) | (_, Err(_)) => false,
                    (Some(user_vsetter), Ok(vsetteres)) => {
                        vsetteres.iter().any(|vsetter| vsetter.name() == user_vsetter)
                    }
                }
            }
            (Self::SemverRange(range), Version::Semver(semver)) => semver.satisfies(range),
            (_, Version::Bypassed | Version::Lts(_) | Version::VSetter(_) | Version::Latest) => false,
    ...
    /// The inferred vsetter for the user version, if it exists.
...
mod tests {
    ...
    fn test_major_to_version() {
        ...
        let result = UserVersion::OnlyMajor(6).to_version(&versions, &EnmConfig::default());
    ...
    fn test_major_minor_to_version() {
        ...
        let result = UserVersion::MajorMinor(6, 0).to_version(&versions, &EnmConfig::default());
    ...
    fn test_semver_to_version() {
        ...
        let result =
            UserVersion::Full(expected.clone()).to_version(&versions, &EnmConfig::default());
