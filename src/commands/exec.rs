...
use crate::user_opted_version::{
    user_opted_version, Error as UserInputError,
};
use crate::config::EnmConfig;
use crate::outln;
use crate::user_version::UserVersion;
use crate::version_uv_reader::UserVersionReader;
...
impl Cmd for Exec {
    ...
    fn apply(self, config: &EnmConfig) -> Result<(), Self::Error> {
        ...
        let applicable_version = user_opted_version(&version, config)
...
pub enum Error {
    ...
    #[error("command not provided. Please provide a command to run as an argument, like {} or {}.\n{} {}", "node".italic(), "bash".italic(), "example:".yellow().bold(), "enm exec --using=12 node --version".italic().yellow())]
    NoBinaryProvided,
}
